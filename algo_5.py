...
def demand_predict(ct, ind, cars, nfwtx, nfwdd):
    # current time, current index, cars data, not_full_wagons(tx and dd)
    not_full_wagon_dd = deepcopy(nfwdd)
    not_full_wagon_tx = deepcopy(nfwtx)
    i = ind
    counts = [0, 0, 0]
    while True:
        if i >= len(cars):
            break
        if cars[i][0] > ct: # Magic number!
            break
        t, curr_car = cars[i]
        ...
        if curr_car.plant == "DD":
            if len(not_full_wagon_dd[d]) == 0:
                ...
                if ret is None:
                    ...
                    not_full_wagon_dd[d][-1].insert_DD(curr_car) 
        i += 1
    # count position [2,3,4,7,8] vacancy
    
    for d in range(3):
        wl = len(not_full_wagon_dd[d])
        for j in range(wl-1):
            for p in [2,3,4,7,8]:
                if not_full_wagon_dd[d][j].loads[p] is None:
                    counts[d] += 1
    return counts
...
while True:
    ...
    if curr_car.plant == "TX":
        skipped = False
        demands = demand_predict(t, i, cars, not_full_wagon_tx, not_full_wagon_dd)
        if not scheduling:
            if sum(demands) >= 6:
                demand_target = deepcopy(demands)
                trunk_target = sum(demands)//6
                count_target = trunk_target * 6
                scheduling = True
        if scheduling:
            code = curr_car.code
            dda = dda_to_int[curr_car.dda]
            if code in ['F52', 'F49', 'F39']:
                if buffer_counts[dda] < demand_target[dda]:
                    buffer[dda].append(curr_car)
                    buffer_counts[dda] += 1
                    skipped = True
            if sum(buffer_counts) == count_target:
                buffer_total = []
                for x in buffer:
                    buffer_total += x
                buffer_total = sorted(buffer_total, key=lambda x: x.t)
                for j in range(trunk_target):
                    trunk_time = buffer_total[6*(j+1)-1].t
                    ...
                    for k in range(6):
                        t_car = buffer_total[6*j+k]
                        new_car_time = trunk_time+7200
                        new_car = Car(t_car.vin, new_car_time, t_car.code, "DD", t_car.uuid, t_car.dda)
                        inserted = False
                        for l in range(i,len(cars)):
                            if new_car_time > cars[l][0]:
                                cars.insert(l+1, (new_car_time, new_car))
                                inserted = True
                                break
                        if not inserted:
                            cars.append((new_car_time, new_car))
                        cars_transit_table.append((buffer_total[6*j+k].uuid, "TK{}".format(trunk_ind)))
                    trunk_ind += 1
                scheduling = False
                buffer_counts = [0,0,0]
                buffer = [[], [], []]
